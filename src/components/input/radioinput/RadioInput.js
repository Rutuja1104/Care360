...
const RadioInput = ({
    ...
    return (
        <label className="radio-label" style={{ ...customStyle, ...(disabled ? styles.disabledStyle : {}) }}>
            <input type="radio" className="radio-input" name={name} value={value} checked={checked} onChange={onChangeCb} disabled={disabled} />
            ...
            {label.suffixLabel && <span style={customStyle ? customStyle : styles.radioLabel}>{label.suffixLabel}</span>}
        </label>
    );
};
