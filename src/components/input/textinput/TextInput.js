...
export default function TextInput({
    ...
    const getErrorStyles = () => {
        if (Object.keys(errors).length > 0 || (isTouched && rules?.required && value?.length == 0)) {
            errors = Object.keys(errors).length !== 0 ? errors : { message: "This field is required" }
            return 'input-box-error-styles'
        } else {
            return {};
        }
    }
        ...
        <div style={{ ...styles.container, ...containerStyles }}>
            {isLabel && (
                <label
                    ...
                    style={label !== ''
                        ? { ...labelStyles, ...styles.label, fontWeight: isCommunicationTab ? 600 : undefined }
                        : { ...labelStyles, ...styles.emptyLabel }
                    }
                >
                    {label} <span style={styles.errorStar}>{rules.required && ' *'}</span>
                </label>
            )}
            {isSubLabel && <label className="subLabel" style={subLabel !== '' ? styles.subLabel : styles.emptySubLabel}>
                {subLabel}
            </label>}
            ...
            <div style={{ ...styles.inputContainer, ...getDisabledStyles() }} className={hideBorder ? "border-0" : null}>
                ...
                <input
                    ...
                    className={`input-textBox ${getErrorStyles()} ${className} ${showSpinners ? ' ' : 'hide-spinners'}`}
                    value={value}
                    name={name}
                    min={min}
                    max={max}
                ...
                {suffixProps.icon && (
                    value && isClearable ? (
                        <div
                            ...
                            style={{ ...styles.iconPosition, ...styles.suffixIcon, ...suffixProps.suffixIconStyles }}
                            onClick={() => suffixProps.suffixClearIconCb(name)}
                        >
                        ...
                        <div
                            className="suffix-icon"
                            style={{ ...styles.iconPosition, ...styles.suffixIcon, ...suffixProps.suffixIconStyles }}
                            onClick={suffixProps.suffixIconCb}
                        >
            ...
            {showErrors &&
                <ErrorsList errors={errors} customStyle={customStyle} />
            }
        </div>
    );
}
